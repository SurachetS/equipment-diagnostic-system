...
class DiagnosisController(http.Controller):
    ...
    def diagnosis_selector(self):
        # ดึงข้อมูล Category
        categories = request.env['equipment.category'].sudo().search([])
    ...
    def get_equipments(self, category_id=None):
        ...
        equipments = request.env['equipment.model'].sudo().search([('category_id', 'child_of', category_id)])
    ...
    def get_surveys(self, equipment_id=None):
        ...
        surveys = request.env['survey.survey'].sudo().search([('equipment_model_id', '=', equipment_id)])
